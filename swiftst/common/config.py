...
def parse_config(configfile):
    ...
    if not c.read(configfile):
        ...
        conf = dict(c.defaults())
            ...
            """Keystone"""
            results['ks_ip'] = conf.get('keystone_ip','172.16.0.3')
            results['ks_port'] = conf.get('keystone_port','35357')
            results['ks_auth_proto'] = conf.get('keystone_auth_proto',
                                                'http')
            results['ks_auth_uri'] = conf.get('keystone_auth_uri',
                                              'http://172.16.0.3:5000/')
            results['ks_admin_tenant'] = conf.get('keystone_admin_tenant',
                                                  'Services')
            results['ks_admin_user'] = conf.get('keystone_admin_user','swift')
            results['ks_admin_key'] = conf.get('keystone_admin_key','noswift')
